...
if($values != "")
{
	...
	$query="SELECT nd.nid,nd.title as long_title, (SELECT field_custom_title_value AS title FROM field_data_field_custom_title WHERE entity_id=nd.nid LIMIT 1 ) as title, nd.created ,us.name,nd.uid, frb.body_value FROM node nd INNER JOIN field_data_body frb ON nd.nid = frb.entity_id 
    ...
    foreach($res as $row)
	{
		...
		$data_array = array(				
							...
							"long_title" => utf8_decode($row['long_title']),
							"name" => utf8_decode($row['name']),
							
							"body_value" => utf8_decode($row['body_value']),
							"link" => stripslashes(utf8_decode('http://liveindiahindi.com/'.$y11)),
